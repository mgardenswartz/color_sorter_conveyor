...
bool SparkFun_APDS9960::wireWriteDataBlock(uint8_t reg, uint8_t *val, unsigned int len)
{
    ...
    status = HAL_I2C_Mem_Write(I2C_handle, APDS9960_I2C_ADDR << 1, reg, I2C_MEMADD_SIZE_8BIT, val, len, HAL_MAX_DELAY);
    if (status != HAL_OK) {
        return false;
    }
...
int SparkFun_APDS9960::wireReadDataBlock(uint8_t reg, uint8_t *val, unsigned int len)
{
    ...
    /* Read block data */
    HAL_StatusTypeDef status = HAL_I2C_Mem_Read(I2C_handle, APDS9960_I2C_ADDR << 1, reg, I2C_MEMADD_SIZE_8BIT, val, len, HAL_MAX_DELAY);
    if (status != HAL_OK) {
        return -1;
    }
