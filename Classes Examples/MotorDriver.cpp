...
void MotorDriver::set_duty(float duty){
	...
	if(duty>=0){
		// Raise pin 1
		HAL_GPIO_WritePin(pin_1_group, pin_1, GPIO_PIN_SET);
		...
		// Raise pin 2
		HAL_GPIO_WritePin(pin_2_group, pin_2, GPIO_PIN_SET);
...
void MotorDriver::initialize(){
	// Motor stop
	HAL_GPIO_WritePin(pin_1_group, pin_1, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(pin_2_group, pin_2, GPIO_PIN_RESET);
}
